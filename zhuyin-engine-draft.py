    ...
    if nm == 1:  # if chord has # for number
    	...
    	if krf not in tones:
    		...
    		if klf in irregularSyllables:
    			...
    			if "Z" in kls:
    				ph = palatalInitials[kls] + medials[klmu] + rimes[kle] + tones[krf]
    			elif klr in irregularRimes:
    				ph = palatalIniitals[kls] + irregularRimes[klr] + tones[krf]
    			else:
    				ph = palatalInitials[kls] + medials[klm] + rimes[kle] + tones[krf]
    		elif kle = "":
    			ph = defaultSyllablesp[klf] + tones[krf]
    		else:
    			ph = simpleInitials[kls] + medials[klm] + rimes[kle] + tones[krf]
    elif klf in irregularSyllables:
    	ph = irregularSyllables[klf]
    elif "I" in klm:
    	if "Z" in kls:
    		ph = palatalInitials[kls] + medials[klmu] + rimes[kle]
    	elif klr in irregularRimes:
    		ph = palatalIniitals[kls] + irregularRimes[klr]
    	else:
    		ph = palatalInitials[kls] + medials[klm] + rimes[kle]
    elif kle = "":
    	ph = defaultSyllablesp[klf]
    else:
    	ph = simpleInitials[kls] + medials[klm] + rimes[kle]
